    ...
    <div className="homemain">
      ...
      <div className="homemain__action">
        {actionData.map((item, index) => {
          return <ActionCard icon={item.icon} title={item.title} description={item.description} key={index} />;
        })}
      </div>
      <h2 className="homemain__subtitle">Recently Edited</h2>
      <div className="homemain__recent">
        {recentData.map((item, index) => {
          return <HomeCard image={item.image} title={item.title} edited={item.edited} author={item.author} key={index} />;
        })}
      </div>
